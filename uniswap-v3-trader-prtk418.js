...
// WEB3 CONFIG
var web3 = new Web3(new Web3.providers.WebsocketProvider(process.env.RPC_URL_WS));
...
var uniswapRouter = new web3.eth.Contract(UNISWAP_QUOTER_ABI, UNISWAP_QUOTER_ADDRESS);
...
const kyberNetProxyContract = new web3.eth.Contract(kyberNetProxyABI, kyberNetProxyAddress);
...
async function checkPair(args) {
  ...
  let outputTokenContract = new web3.eth.Contract(ERC20_ABI, outputTokenAddress);
  let outputTokenDecimal = await outputTokenContract.methods.decimals.call().call();

  let uniswapResult = await uniswapRouter.methods.quoteExactInputSingle(inputTokenAddress, outputTokenAddress, 3000, inputAmount, 0).call();
  let kyberResult = await kyberNetProxyContract.methods.getExpectedRate(inputTokenAddress, outputTokenAddress, inputAmount).call();  
  ...
  console.table([{
    ...
    'Uniswap Return': uniswapResult/(10**outputTokenDecimal),
...
async function monitorPrice() {
  ...
  try {
    ...
    await checkPair({
      inputTokenSymbol: 'WETH',
      inputTokenAddress: '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2',
      outputTokenSymbol: 'COMP',
      outputTokenAddress: '0xc00e94cb662c3520282e6f5717214004a7f26888',
      inputAmount: web3.utils.toWei('1', 'ETHER'),
    })
  } catch (error) {
    console.error(error)
